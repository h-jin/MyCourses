		...
		<div class="col-xs-12 col-sm-9">
			<div class="jumbotron">
				...
				<table class="table table-hover">
					...
					<tbody>
						<c:forEach items="${programs}" var="cs">
					   		<tr>
								<td > <c:out value="${cs.name}" /></td>
								<td > <c:out value="${cs.deptid}" /></td>
								<td > <c:out value="${cs.fees}" /></td>
								<td > <a type="button" class="btn btn-primary btn-xs" href="AccountController?action=update_fees&name=<c:out value="${cs.name}"/>">update</a></td>
