	...
	<div class="row row-offcanvas row-offcanvas-right">
		<div class="col-xs-12 col-sm-9">
			<div class="jumbotron">
        		...
        		<c:if test="${fn:contains(str,'No_Record')}">No Record Found!</c:if><br>
        		...
        		<c:if test="${fn:contains(str,'OK')}">Personal information has been updated!</c:if><br>  
 				<c:if test="${fn:contains(str,'Invalid1')}">Email already exists!</c:if><br>
 				<c:if test="${fn:contains(str,'Invalid2')}">Invalid Email Address!</c:if><br>  
 				<c:if test="${fn:contains(str,'pass_unmatch')}">Wrong Old Password!</c:if><br>  				
 				<c:if test="${fn:contains(str,'Password')}">Password Changed!</c:if><br>
 				<c:if test="${fn:contains(str,'InvalidP')}">Invalid Password!</c:if><br>  
 				<c:if test="${fn:contains(str,'Unmatch')}">Not Match!</c:if><br> 
