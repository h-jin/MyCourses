...
<title>Registered Course Details</title>
</head>
<body>
...
<div class="container">
	<div class="row row-offcanvas row-offcanvas-right">
		...
		<!-- <a type="button" class="btn btn-primary btn-xs pull-right" href="RegistrationController?action=display&user=<%=session.getAttribute("user")%>">back</a>  -->
			<c:set var="time1"><fmt:formatDate value="${deadline.dpdeadline}" pattern="yyyy-MM-dd HH:mm:ss"/></c:set> 
			<c:set var="time2"><fmt:formatDate value="${now}" pattern="yyyy-MM-dd HH:mm:ss" /></c:set> 
						
		    <c:if test="${time1 > time2 }"> <td ><a type="button" class="btn btn-primary btn-xs" href="RegistrationController?action=delete&id=<c:out value="${course.courseid}"/>&name=<%=session.getAttribute("user")%>" onclick="javascript:return myFunction()" >drop</a></td>
			</c:if>
